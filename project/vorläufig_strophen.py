...
class Constram(Scene):
    def construct(self):
        ...
        semikreis = Arc(
            radius=r, start_angle=0, angle=PI, arc_center=M.get_center(), color=TXTCOL
        )
        ...
        dashed1 = DashedLine(
            S.get_center(), G.get_center(), color=TXTCOL, stroke_width=3
        )
        dashedam = DashedLine(
            M.get_center(), X.get_center(), color=AMCOL, stroke_width=2
        )
        ...
        b1 = Brace(
            firsta,
            direction=firsta.copy().rotate(PI / 2).get_unit_vector(),
            color=TXTCOL,
        )
        b2 = Brace(
            firstb,
            direction=firstb.copy().rotate(PI / 2).get_unit_vector(),
            color=TXTCOL,
        )
        b1text = b1.get_tex("a")
        ...
        abr = Brace(
            sega,
            direction=firsta.copy().rotate(3 * PI / 2).get_unit_vector(),
            color=TXTCOL,
        )
        bbr = Brace(
            segb,
            direction=firstb.copy().rotate(3 * PI / 2).get_unit_vector(),
            color=TXTCOL,
        )
        ...
        rightG = Angle(
            Line(G.get_center(), S.get_center()),
            Line(G.get_center(), X.get_center()),
            radius=0.3,
            other_angle=True,
            dot=True,
            dot_color=RIGHTANGLECOL,
        ...
        term13 = MathTex(
            r"|\overline{SM}| = ({{\frac{b-a}{2}}})^2 + ({{\frac{b-a}{2}}})^2 ",
            color=TXTCOL,
        ).shift(UP * 3)
